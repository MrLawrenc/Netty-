...
public class PoolBlockingQueue<E> extends AbstractQueue<E>
     ...
     */
    transient PoolBlockingQueue.Node<E> head;

    /**
     * Tail of linked list.
     * Invariant: last.next == null
     */
    private transient PoolBlockingQueue.Node<E> last;
     ...
     * @param node the node
     */
    private void enqueue(PoolBlockingQueue.Node<E> node) {
    ...
    private E dequeue() {
        ...
        PoolBlockingQueue.Node<E> h = head;
        PoolBlockingQueue.Node<E> first = h.next;
        h.next = h; // help GC
        head = first;
        E x = first.item;
        first.item = null;
    ...
    public PoolBlockingQueue(int capacity) {
        ...
        this.capacity = capacity;
        last = head = new PoolBlockingQueue.Node<E>(null);
    }
     ...
     */
    public PoolBlockingQueue(Collection<? extends E> c) {
        this(Integer.MAX_VALUE);
        final ReentrantLock putLock = this.putLock;
        ...
        try {
            ...
            for (E e : c) {
                ...
                enqueue(new PoolBlockingQueue.Node<E>(e));
                ++n;
            }
            count.set(n);
        } finally {
    ...
    public void put(E e) throws InterruptedException {
        ...
        final int c;
        final PoolBlockingQueue.Node<E> node = new PoolBlockingQueue.Node<E>(e);
        final ReentrantLock putLock = this.putLock;
     ...
     */
    void unlink(PoolBlockingQueue.Node<E> p, PoolBlockingQueue.Node<E> pred) {
        // assert putLock.isHeldByCurrentThread();
        // assert takeLock.isHeldByCurrentThread();
    ...
    public boolean remove(Object o) {
        ...
        fullyLock();
        try {
            for (PoolBlockingQueue.Node<E> pred = head, p = pred.next;
                 p != null;
                 pred = p, p = p.next) {
                if (o.equals(p.item)) {
                    unlink(p, pred);
    ...
    public boolean contains(Object o) {
        ...
        fullyLock();
        try {
            for (PoolBlockingQueue.Node<E> p = head.next; p != null; p = p.next)
                if (o.equals(p.item))
                    return true;
            return false;
        } finally {
    ...
    public void clear() {
        fullyLock();
        try {
            for (PoolBlockingQueue.Node<E> p, h = head; (p = h.next) != null; h = p) {
                h.next = h;
                p.item = null;
            }
            head = last;
    ...
    public int drainTo(Collection<? super E> c, int maxElements) {
        ...
        try {
            ...
            try {
                while (i < n) {
                    PoolBlockingQueue.Node<E> p = h.next;
                    c.add(p.item);
                    p.item = null;
                    h.next = h;
                    h = p;
     ...
     */
    PoolBlockingQueue.Node<E> succ(PoolBlockingQueue.Node<E> p) {
        if (p == (p = p.next))
            p = head.next;
        return p;
    }
    ...
    public Iterator<E> iterator() {
        return new PoolBlockingQueue.Itr();
    }
    ...
    private class Itr implements Iterator<E> {
        private PoolBlockingQueue.Node<E> next;           // Node holding nextItem
        private E nextItem;             // next item to hand out
        private PoolBlockingQueue.Node<E> lastRet;
        private PoolBlockingQueue.Node<E> ancestor;       // Helps unlink lastRet on remove()
        ...
        public void forEachRemaining(Consumer<? super E> action) {
            ...
            do {
                ...
                try {
                    if (es == null) {
                        p = p.next;
                        for (PoolBlockingQueue.Node<E> q = p; q != null; q = succ(q))
        ...
        public void remove() {
            PoolBlockingQueue.Node<E> p = lastRet;
            if (p == null)
    ...
    private final class LBQSpliterator implements Spliterator<E> {
        ...
        PoolBlockingQueue.Node<E> current;    // current node; null until initialized
        ...
        public Spliterator<E> trySplit() {
            ...
            if (!exhausted &&
                ...
                int i = 0;
                PoolBlockingQueue.Node<E> p = current;
                fullyLock();
                try {
    ...
    public Spliterator<E> spliterator() {
        return new PoolBlockingQueue.LBQSpliterator();
    }
     ...
     */
    void forEachFrom(Consumer<? super E> action, PoolBlockingQueue.Node<E> p) {
     ...
     */
    PoolBlockingQueue.Node<E> findPred(PoolBlockingQueue.Node<E> p, PoolBlockingQueue.Node<E> ancestor) {
        // assert p.item != null;
        if (ancestor.item == null)
            ancestor = head;
        // Fails with NPE if precondition not satisfied
        for (PoolBlockingQueue.Node<E> q; (q = ancestor.next) != p; )
            ancestor = q;
        return ancestor;
    }
    ...
    private boolean bulkRemove(Predicate<? super E> filter) {
        ...
        PoolBlockingQueue.Node<E> p = null, ancestor = head;
        PoolBlockingQueue.Node<E>[] nodes = null;
        int n, len = 0;
        do {
                ...
                if (nodes == null) {  // first batch; initialize
                    ...
                    for (PoolBlockingQueue.Node<E> q = p; q != null; q = succ(q))
                        if (q.item != null && ++len == 64)
                            break;
                    nodes = (PoolBlockingQueue.Node<E>[]) new PoolBlockingQueue.Node<?>[len];
                }
                for (n = 0; p != null && n < len; p = succ(p))
                    nodes[n++] = p;
            } finally {
            ...
            if (deathRow != 0) {
                ...
                try {
                    for (int i = 0; i < n; i++) {
                        final PoolBlockingQueue.Node<E> q;
                        if ((deathRow & (1L << i)) != 0L
                                && (q = nodes[i]).item != null) {
